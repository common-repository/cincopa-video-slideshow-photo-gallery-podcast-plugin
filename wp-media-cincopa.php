...
{
	header('Location: '.cincopa_url().'/media-platform/start.aspx?ver='.cincopa_plugin_ver().cincopa_afc().'&rdt='.urlencode(cincopa_selfURL()));
	exit;
}
...
function cincopa_tag($fid)
{
	return cincopa_plugin_callback(array($fid));
}
...
function cincopa_plugin_callback($match)
{
	$uni = uniqid('');
	$ret = '
<!-- Powered by Cincopa WordPress plugin '.cincopa_plugin_ver().': http://www.cincopa.com/media-platform/wordpress-plugin.aspx -->
...
function cincopa_async_plugin_callback($match)
{
	$uni = uniqid('');
	$ret = '
<!-- Powered by Cincopa WordPress plugin '.cincopa_plugin_ver().' (async engine): http://www.cincopa.com/media-platform/wordpress-plugin.aspx -->
...
function cincopa_plugin($content)
{
	...
	if ($cincopa_async == 'async')
		return (preg_replace_callback(CINCOPA_REGEXP, 'cincopa_async_plugin_callback', $content));
	else
...
add_action ( 'bp_get_activity_content_body', 'cincopa_plugin' );
add_action ( 'bp_get_the_topic_post_content', 'cincopa_plugin' );
...
function cincopa_dashboard_content()
{
	...
	echo "<iframe src='http://www.cincopa.com/media-platform/wordpress-dashboard-content.aspx?ver=".cincopa_plugin_ver().cincopa_afc()."&src=".urlencode(cincopa_selfURL())."' width='100%' height='370px' scrolling='no'></iframe>";
...
//	add_management_page('Test Manage', 'Test Manage', 8, 'testmanage', 'mt_manage_page');
	...
	if(function_exists('add_menu_page'))
	{
		...
		add_submenu_page(__FILE__, 'Manage Galleries', 'Manage Galleries', 8, 'sub-page', 'mt_cincopa_sublevel_monitor');

		add_submenu_page(__FILE__, 'Media Library', 'Media Library', 8, 'sub-page1', 'mt_cincopa_sublevel_library');
...
//	}
	...
	{
		update_site_option('cincopa_welcome_notice', cincopa_plugin_ver());
	...
	{
		update_site_option('premiumpress_notice', cincopa_plugin_ver());
	}
...
?>
									...
									<tr style="width:100%;">
										<th valign="top" scrope="row" colspan=2>
											Note:
<ol>
<li>Use this PHP code to add a gallery directly to your template : <br>&nbsp;&nbsp;&nbsp; <i>&lt;?php echo cincopa_tag("GALLERY ID"); ?&gt;</i></li>
</ol>
										</th>
									</tr>

<?php						if (get_site_option('premiumpress_notice') != cincopa_plugin_ver())
									{	?>
...
function mt_cincopa_toplevel_page() {
    echo "<iframe src='http://www.cincopa.com/media-platform/start.aspx?ver=".cincopa_plugin_ver().cincopa_afc()."&src=".urlencode(cincopa_selfURL())."' width='98%' height='2000px'></iframe>";
}

function mt_cincopa_sublevel_create() {
    echo "<iframe src='http://www.cincopa.com/media-platform/wizard_name.aspx?ver=".cincopa_plugin_ver().cincopa_afc()."&src=".urlencode(cincopa_selfURL())."' width='98%' height='2000px'></iframe>";
}

function mt_cincopa_sublevel_monitor() {
    echo "<iframe src='http://www.cincopa.com/media-platform/wizard_edit.aspx?ver=".cincopa_plugin_ver().cincopa_afc()."&src=".urlencode(cincopa_selfURL())."' width='98%' height='2000px'></iframe>";
}

function mt_cincopa_sublevel_library() {
    echo "<iframe src='http://www.cincopa.com/media-platform/wizard2/library.aspx?ver=".cincopa_plugin_ver().cincopa_afc()."&src=".urlencode(cincopa_selfURL())."' width='98%' height='2000px'></iframe>";
}
...
{ ?>
			<div id="message" class="updated fade">
				...
				<p>
					On every post page (above the text box) you'll find this <img src="<?php echo cincopa_pluginURI() ?>/media-cincopa.gif"  /> icon, click on it to start or use sidebar Widgets (Appearance menu).
				</p>
				<p>
...
if (get_site_option('cincopa_welcome_notice') != cincopa_plugin_ver())
	add_action( 'admin_notices', 'cp_activation_notice' );
